...
} else if (currentUser.role === 'owner' && location.pathname.endsWith('staff.html')) {
...
// ═══════════════ NAVIGATION ═══════════════
const pages = ['dashboard', 'appointments', 'doctors', 'patients', 'services'];
...
async function loadDashboard() {
    ...
    if (stats) {
        if (document.getElementById('stat-appt-total')) {
...
async function loadTodayAppointments() {
    const rawData = await apiFetch('/appointments/today/');
...
async function loadChart(days) {
    const data = await apiFetch('/appointments/stats/'); // In real app, filter by days
    const dates = []; const counts = [];
    for (let i = parseInt(days) - 1; i >= 0; i--) {
        const d = new Date(); d.setDate(d.getDate() - i);
        dates.push(d.toLocaleDateString(lang, { month: 'short', day: 'numeric' }));
...
function renderStatusChart(breakdown) {
    ...
    // Fallback if no data
    const labels = breakdown ? Object.keys(breakdown).map(k => T[lang][k] || k) : ['Tasdiqlangan', 'Kutilmoqda', 'Yakunlangan'];
    const values = breakdown ? Object.values(breakdown) : [10, 5, 15];
...
async function loadTopDoctors() {
    const data = await apiFetch('/analytics/clinic/');
    ...
    list.innerHTML = data.top_doctors.slice(0, 5).map((d, i) => `
        <div class="flex items-center gap-3" style="padding:15px 0;border-bottom:1px solid var(--gray-100)">
          ...
          <div style="flex:1">
            ...
            <div style="font-size:12px;color:var(--gray-600)">${d.total_appointments || 0} navbat</div>
          </div>
...
async function loadAnalytics() {
    ...
    const data = await apiFetch(`/analytics/clinic/?period=${period}`);
        ...
        <tr>
            <td>
                <div class="flex items-center gap-2">
                    <div class="avatar" style="background:var(--gray-100);color:var(--dark)">${(s.first_name || 'X')[0]}</div>
                    <div>
                        <div style="font-weight:600">${s.first_name || ''} ${s.last_name || ''}</div>
                        <div style="font-size:11px;color:var(--gray-500)">ID: ${s.id?.slice(0, 8) || ''}</div>
                    </div>
                </div>
            </td>
            <td><strong>${s.username || ''}</strong></td>
            <td>${s.phone || '—'}</td>
            <td><span class="badge ${s.role === 'Asoschi' ? 'badge-blue' : 'badge-green'}">${s.role || 'Admin'}</span></td>
        </tr>
    `).join('');
}
